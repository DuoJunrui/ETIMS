...
import cn.duojunrui.etims.system.service.DeptService;
...
public class DeptController extends BaseController {
    ...
    @PostMapping("/save")
    @RequiresPermissions("system:sysDept:add")
    public ResultUtils save(DeptDO sysDept) {
    ...
    @RequiresPermissions("system:sysDept:edit")
    public ResultUtils update(DeptDO sysDept) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultUtils.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
    ...
    public ResultUtils remove(Long deptId) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultUtils.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
        ...
        if(sysDeptService.count(map)>0) {
            return ResultUtils.error(1, "包含下级部门,不允许修改");
        }
        if(sysDeptService.checkDeptHasUser(deptId)) {
            ...
            return ResultUtils.error(1, "部门包含用户,不允许修改");
        }
        return ResultUtils.error();
    }
    ...
    public ResultUtils remove(@RequestParam("ids[]") Long[] deptIds) {
        if (Constant.DEMO_ACCOUNT.equals(getUsername())) {
            return ResultUtils.error(1, "演示系统不允许修改,完整体验请部署程序");
        }
